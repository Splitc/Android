...
public class GooglePlaceAutocompleteApi implements Callable<ArrayList<String>> {
    ...
    ArrayList<GooglePlaceAutocompleteObject> resultObjectList = null;
    String key = null;
    ...
    public ArrayList<GooglePlaceAutocompleteObject> autocomplete (final String input, String typeParam, ArrayList<GooglePlaceAutocompleteObject> autocompleteStaticData) {
        java.net.URL url;
    ...
    private void creatResultArrayList(String response, ArrayList<GooglePlaceAutocompleteObject> autocompleteStaticData, String input) {
        try {
            JSONObject jsonObj = new JSONObject(response);
            JSONArray predsJsonArray = jsonObj.getJSONArray("predictions");
            resultList = new ArrayList<String>(predsJsonArray.length());
            resultObjectList = new ArrayList<GooglePlaceAutocompleteObject>(predsJsonArray.length());
            for (int i = 0; i < predsJsonArray.length(); i++) {
                ...
                if (!resultList.contains(suggestionText)) {
                    ...
                    GooglePlaceAutocompleteObject temp = new GooglePlaceAutocompleteObject();
                    temp.setDisplayName(suggestionText);
                    temp.setId(predsJsonArray.getJSONObject(i).getString("place_id"));
                    resultObjectList.add(temp);
                }
            }
            Comparator<GooglePlaceAutocompleteObject> comparator = GooglePlaceAutocompleteComparator.getComparator();
            if (autocompleteStaticData != null && autocompleteStaticData.size() > 0) {
                int flag = 0;
                for (GooglePlaceAutocompleteObject temp : autocompleteStaticData) {
            ...
            GooglePlaceAutocompleteObject poweredByGoogleObject = new GooglePlaceAutocompleteObject();
            poweredByGoogleObject.setDisplayName("PoweredByGoogle");
            poweredByGoogleObject.setId(null);
